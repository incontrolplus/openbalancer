...
- **CompanyBook.bg Clean Light Design & Zero Dark Disconnect Invariant**: In all public-facing light theme pages and dashboards, the canvas must strictly adhere to the CompanyBook.bg clean palette: `#ffffff` base cards, `#f8fafc` background canvas, `#e2e8f0` 1px crisp borders, `#0f172a` primary headings (`Space Grotesk`), `#475569` secondary body text (`Inter`), and `#2563eb` royal blue interactive accents. NEVER leave isolated, discordant dark/black blocks (`#0d1117`, `rgba(15, 23, 42, 0.9)`, dark simulation stages, or charcoal banners) inside a light-theme layout. All terminal cards, tab switchers, telemetry panels, topology maps, footers, and mobile drawers must maintain harmonious light backgrounds and maximum contrast.
- **High-Contrast Light-Theme Code Syntax (GitHub Light) Invariant**: When rendering code snippets, terminal views, or configuration builders on light backgrounds (`#f8fafc` / `#ffffff`), NEVER use dark-theme pastel syntax colors (`#a5d6ff`, `#7ee787`, `#ff7b72`) which produce low contrast (< 3:1). Always use the GitHub Light high-contrast color token standard: `#cf222e` (crimson keywords), `#0550ae` (vibrant navy properties), `#0a3069` (deep navy strings), `#116329` (forest green services/types) achieving WCAG AAA contrast ratio (>= 7:1) on `#f8fafc`.
- **Uniform Multi-Location Brand Logo & Cache-Buster Invariant**: Brand logos rendered across distinct viewport locations (Header Navigation, Mobile Slide-out Drawer, Footer, Subpages) must use identical SVG assets (`assets/logo.svg`), identical explicit dimensions (`width="36" height="36" style="width:36px!important;height:36px!important;object-fit:contain!important;"`), and synchronized cache-buster query parameters (`logo.svg?v=X.Y.Z`). This prevents stale browser cache rendering where drawers or footers display outdated/blurry logo variations while headers display new ones.
- **Default Bulgarian (`bg`) First Localization Invariant**: All multi-lingual OpenBalancer and Finans Protect web properties must initialize `currentLang = 'bg'` by default on fresh sessions (when `localStorage.getItem('openbalancer_lang')` is null or empty). The live language switcher (`BG` ↔ `EN`) must dynamically translate all navigation links, metrics, status badges, and modals without requiring page reloads.
